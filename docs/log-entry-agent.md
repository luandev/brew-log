...
7. **Never commit or write directly to `main`.** All repository changes must follow the review workflow below.
...
2. **Summarize before writing.** Present the user with a concise text summary of the proposed changes, including affected files and any stage/schedule implications.
